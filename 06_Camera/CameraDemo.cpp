...
void CameraDemo::Initialize()
{
	...
	mTextureContainer.LoadTexture("res\\container.jpg");// = SOIL_load_image("res/container.jpg", &imageWidth, &imageHeight, 0, SOIL_LOAD_RGB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, mTextureContainer.GetWidth(), mTextureContainer.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, mTextureContainer.GetData());
	...
	mTextureAwesomeFace.LoadTexture("res\\awesomeface.png");// = SOIL_load_image("res/awesomeface.png", &imageWidth2, &imageHeight2, 0, SOIL_LOAD_RGB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, mTextureAwesomeFace.GetWidth(), mTextureAwesomeFace.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, mTextureAwesomeFace.GetData());
	glGenerateMipmap(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, 0);
}
